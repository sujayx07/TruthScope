...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "processText" && tabId) {
    console.log(
      `📝 [Tab ${tabId}] Received text for analysis:`,
      message.data.url
    );
    ...
    fetch(TEXT_ANALYSIS_URL, {
      ...
      .then(async (response) => {
        console.log(
          `[Tab ${tabId}] Text Analysis API Response Status:`,
          response.status
        );
        if (!response.ok) {
      ...
      .then((result) => {
        ...
        if (result && result.error) {
          console.error(
            `[Tab ${tabId}] Backend returned an error: ${result.error}`
          );
          ...
          chrome.tabs
            ...
            .catch((err) =>
              console.log(
                `[Tab ${tabId}] Error sending analysisError to content script:`,
                err
              )
            );
        ...
        if (result && result.textResult) {
          ...
          chrome.tabs
            ...
            .catch((err) =>
              console.log(
                `[Tab ${tabId}] Error sending analysisComplete to content script:`,
                err
              )
            );

          if (
            result.textResult.highlights &&
            result.textResult.highlights.length > 0
          ) {
            chrome.tabs
              .sendMessage(tabId, {
              ...
              .catch((err) =>
                console.log(
                  `[Tab ${tabId}] Error sending highlights to content script:`,
                  err
                )
              );
          }
          ...
          chrome.runtime
            ...
            .catch((err) =>
              console.log(
                `Error notifying UI components of analysis completion:`,
                err
              )
            );
          ...
          console.error(
            `[Tab ${tabId}] Backend response did not contain expected 'textResult'. Response:`,
            result
          );
          if (!processingState[tabId]) processingState[tabId] = {};
          processingState[tabId].textResult = {
          ...
          chrome.tabs
            ...
            .catch((err) =>
              console.log(
                `[Tab ${tabId}] Error sending analysisError (invalid format) to content script:`,
                err
              )
            );
          sendResponse({
      ...
      .catch((error) => {
        console.error(
          `[Tab ${tabId}] ❌ Error during text analysis fetch:`,
          error
        );
        if (!processingState[tabId]) processingState[tabId] = {};
  ...
  if (message.action === "processMediaItem" && tabId) {
    ...
    console.log(
      `🖼️ [Tab ${tabId}] Received ${mediaType} for analysis: ${mediaUrl} (ID: ${mediaId})`
    );
    ...
    if (!mediaUrl || !mediaType || !mediaId) {
      ...
      chrome.tabs
        ...
        .catch((err) =>
          console.log(
            `[Tab ${tabId}] Error sending invalid data error to content script:`,
            err
          )
        );
    ...
    switch (mediaType.toLowerCase()) {
      ...
      default:
        console.warn(
          `[Tab ${tabId}] Unsupported media type: ${mediaType}. Cannot analyze.`
        );
        ...
        chrome.tabs
          ...
          .catch((err) =>
            console.log(
              `[Tab ${tabId}] Error sending unsupported type error to content script:`,
              err
            )
          );
        sendResponse({
    ...
    console.log(
      `[Tab ${tabId}] Attempting to fetch media analysis from: ${targetUrl}`
    ); // <-- Add log

    fetch(targetUrl, {
      // <-- Use the determined targetUrl
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
      ...
      .then(async (response) => {
        console.log(
          `[Tab ${tabId}] Media Item Analysis API Response Status (${mediaUrl}):`,
          response.status
        );
      ...
      .then((result) => {
        console.log(
          `[Tab ${tabId}] Media item analysis result for ${mediaUrl}:`,
          result
        );
        ...
        const summaryText =
          result?.analysis_summary || "Analysis complete, no summary provided.";

        // Update to display ParsedText and AI-generated score in percentage
        if (result?.manipulated_media?.length > 0) {
          const media = result.manipulated_media[0];
          const extractedText = media.ParsedText || "No text extracted";
          const aiGeneratedScore = (media.ai_generated * 100).toFixed(2) + "%";

          chrome.tabs.sendMessage(tabId, {
            action: "displayMediaAnalysis",
            data: {
              mediaId: mediaId,
              summary: `Extracted Text: ${extractedText}, AI-Generated Score: ${aiGeneratedScore}`,
            },
          });
        } else {
          chrome.tabs.sendMessage(tabId, {
            ...
            data: {
              mediaId: mediaId,
              summary: "Analysis complete, no summary provided.",
            },
          });
        }
        ...
        chrome.runtime
          ...
          .catch((err) =>
            console.log(
              `Error notifying UI components of media item analysis completion:`,
              err
            )
          );
      ...
      .catch((error) => {
        ...
        console.error(
          `[Tab ${tabId}] ❌ Error during media item analysis fetch for ${mediaUrl}. Full Error:`,
          error
        );
        ...
        chrome.tabs
          ...
          .catch((err) =>
            console.log(
              `[Tab ${tabId}] Error sending displayMediaAnalysis (error) to content script:`,
              err
            )
          );
